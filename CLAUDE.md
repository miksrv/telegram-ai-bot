...
4. A background daemon thread loops on the queue until `STARMAP_MAX_WAIT` (default 120s). On the first `queued` ack it posts a transient "Начал процесс генерации карты…" status message **as a reply to the command**. On `ok` it deletes that status message and posts the chart **as a document** (`send_document`, not a compressed photo) replying to the command — reading `image_path` from the shared filesystem, falling back to decoding `image_base64`. On `error`/timeout it deletes the status message and replies with the message. The status-message lifecycle (`safe_reply`/`safe_delete`) lives in `handlers/delivery.py` and is shared with `/photo`
